// --- CONFIGURATION ---
vec4 TRAIL_COLOR = vec4(iCurrentCursorColor.rgb, 0.42); // lower alpha keeps motion visible but calm
const float DURATION = 0.14; // total animation time
const float TRAIL_SIZE = 0.55; // 0.0 = all corners move together. 1.0 = max smear (leading corners jump instantly)
const float THRESHOLD_MIN_DISTANCE = 2.0; // min distance to show trail (units of cursor height)
const float BLUR = 0.8; // blur size in pixels (for antialiasing)
const float TRAIL_THICKNESS = 0.72;  // 1.0 = full cursor height, 0.0 = zero height, >1.0 = funky aah
const float TRAIL_THICKNESS_X = 0.62;
